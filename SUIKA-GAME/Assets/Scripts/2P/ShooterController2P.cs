...
public class ShooterController2P : MonoBehaviour
{
    ...
    {
        OnKeyCursor(); // 키보드 커서를 입력받는 메소드
        OnKeyInput(); // 키보드 버튼을 입력받는 메소드
    }

    private void OnKeyCursor() // 키보드 커서 입력을 통해 슈터를 이동시키는 메소드
    {
        float _horizontal = Input.GetAxis("Horizontal"); // Mouse X => (만약 키보드 입력이면...) Horizontal, 마우스 커서 입력 실수 값
    ...
    private void OnKeyInput() // 마우스 좌,우버튼 입력받는 메소드
    {
        if (isShootDelayed == false && Input.GetKeyDown(KeyCode.Space)) // 마우스 왼쪽 버튼을 눌렀을 때
        {
    ...
    private void OnSetPlanet() // 행성 초기 설정 메소드
    {
        ...
        _planetController.is2Played = true; // 2P 상태를 참으로 설정한다
    ...
    private void OnShoot(GameObject _planet) // 행성 발사 메소드
    {
        ...
        ShowPrevPlanetUI2P.OnSpriteChange(currentPlanetNumber); // 스프라이트 체인지 상태를 true로 변경
    }
