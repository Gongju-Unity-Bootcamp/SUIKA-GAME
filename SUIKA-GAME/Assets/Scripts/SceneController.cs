...
public class SceneController : MonoBehaviour
{
    ...
    public void OnStart2PClick() // 스타트 2P 버튼을 눌렀을 때의 메소드
    {
        PlayerPrefs.SetInt("Score", 0); // 스코어 키로 스코어 변수를 찾아 삭제한다
        PlayerPrefs.SetInt("Score2P", 0); // 스코어 키로 스코어 변수를 찾아 삭제한다
        SoundManager.Play.PlayEffect("MouseClick"); // 사운드 이름으로 사운드 출력
        SceneManager.LoadScene("Ingame2P"); // 인게임 씬을 로드한다
    }
    ...
    public void OnExitClick() // 종료 버튼을 눌렀을 때의 메소드
    {
        SoundManager.Play.PlayEffect("MouseClick"); // 사운드 이름으로 사운드 출력
        Application.Quit(); // 응용 프로그램을 즉시 종료한다
    }

    public void OnBackClick() // 취소 버튼을 눌렀을 때의 메소드
    {
        SoundManager.Play.PlayEffect("MouseClick"); // 사운드 이름으로 사운드 출력
